...
std::shared_ptr<bmpf::NodeGridPathFinder> pathFinder;
...
void testPath(std::vector<double> &start, std::vector<double> &end) {
    ...
    std::vector<std::vector<double>> findingPoses = pathFinder->getAllProcessedStates();
    ...
    assert(pathFinder->simpleCheckPath(path, pathFinder->getMaxDist()));
...
int main() {
    bmpf::infoMsg("test one direction sync finder");

    std::shared_ptr<bmpf::Scene> scene = std::make_shared<bmpf::Scene>();
    scene->loadFromFile("../../../../config/murdf/4robots.json");

    pathFinder = std::make_shared<bmpf::OneDirectionSyncPathFinder>(
            scene, false, 1000, 10, 3000, 5, 1, 8
    );
